    ...
    //   }
      ...
      // {
         while (it.hasNext())
         {
           ...
           if ("-debug".equals(opt)) {
		       if (!(it.hasNext())) {
			         throw new IllegalArgumentException("Argument -debug missing required value");
			       }
           ...
           if ("-h".equals(opt)) {
             ...
             if (this.infile != null) {
               throw new IllegalArgumentException("Argument -i already specified");
             }
             if (!(it.hasNext())) {
               throw new IllegalArgumentException("Argument -i missing required value");
             }
             ...
             if (this.species != null) {
               throw new IllegalArgumentException("Argument -s already specified");
             }
             if (!(it.hasNext())) {
               throw new IllegalArgumentException("Argument -s missing required value");
             }
             ...
             if (!(it.hasNext())) {
               throw new IllegalArgumentException("Argument -r missing required value");
             }
             ...
             LOG.info(new Object[] { "Reaction types: " + this.reactionTypes });
           }
					else if ("-rfile".equals(opt)) {
             if (this.reactionSmarts != null) {
               throw new IllegalArgumentException("Argument -rfile already specified");
             }
             if (!(it.hasNext())) {
               throw new IllegalArgumentException("Argument -rfile missing required file name");
             }
             String fileName = (String)it.next();
             readReactionSmarts(fileName);
             LOG.info(new Object[] { "Reaction SMARTS: " + this.reactionSmarts });
           }
           else if ("-include-multistep".equals(opt)) {
             if (this.includeMultiStep != null) {
               throw new IllegalArgumentException("Argument -include-multistep already specified");
             }
             this.includeMultiStep = Boolean.TRUE;
           }
           else if ("-include-norc".equals(opt)) {
             if (this.includeNoRc != null) {
               throw new IllegalArgumentException("Argument -include-norc already specified");
             }
             this.includeNoRc = Boolean.TRUE;
           }
           else if ("-log".equals(opt)) {
             if (!(it.hasNext())) {
               throw new IllegalArgumentException("Argument -log missing required value");
             }
             FileAppender logfile = new FileAppender(new SimpleLayout(), (String)it.next());
         ...
         if (this.infile == null) {
           throw new IllegalArgumentException("Required argument -i not specified");
         }
         if (this.outfile == null)
           throw new IllegalArgumentException("Required argument -o not specified");
//       }
   }
private void readReactionSmarts(String fileName) throws Exception {
   ...
   private void printUsage()
   {
     ...
     System.out.println("-h                          display full help message");
     ...
     System.out.println("-include-norc               include transformations with no RC (optional)");
     System.out.println("-debug <input file>         run debug viewer (takes precedence over other options)");
     System.out.println();
   }
   ...
   private void printHelp() {
     ...
     System.out.println("      RXN:VARIATION(1):LITREF(*):ANIMAL(*):SPECIES");
     System.out.println();
     System.out.println("Species Filter");
     System.out.println("Species filter is a complete word e.g. 'human' or 'rat' to match");
     System.out.println("against the species field.");
     System.out.println();
     System.out.println("Reaction Type Filter");
     System.out.println("Reaction type filter can be either a comma separated list of MetaPrint2D");
     ...
     System.out.println("Or a SMIRKS pattern. In the SMIRKS pattern mapped atoms are conserved");
     System.out.println("between the reactant and product, and unmapped atoms are deleted or");
     System.out.println("added, as appropriate. A match of all added/deleted atoms is not required");
     System.out.println("only those adjacent to the reaction centre; this allows defininitions");
     ...
     System.out.println("By default, multi-step transformations are ignored. They can be included");
     System.out.println("through the -include-multistep flag.");
     System.out.println();
     System.out.println("No Reaction Centre");
     System.out.println("By default, transformations with no reaction centres identified (after");
     System.out.println("the reaction type filter has been applied) are ignored. They can be included");
     System.out.println("through the -include-norc flag.");
     System.out.println();
     System.out.println("------------------------------------------------------------");
     System.out.println("  Author: Sam Adams <sam.adams@cantab.net>");
     System.out.println("  Developed by:");
     System.out.println("  Sam Adams, Lars Carlsson, Scott Boyer, Robert Glen");
     System.out.println("  AstraZeneca, Molndal  /  University of Cambridge");
     System.out.println("------------------------------------------------------------");
     System.out.println();
   }
...
private void run()
     throws Exception
   {
     ...
     LOG.info(String.format("Maximum available memory: %.1fMB%n", new Object[] { Double.valueOf(maxMem) }));
...
//     ReactionTypes rtypes = MetaPrintReactionTypes.RXNTYPES;
     ...
     else
     {
       ...
       try {
         ...
         is = new FileInputStream(infile);
				...
				if(this.reactionSmarts == null){
					...
					metaboliteHandler = new ReactionSmartsHandler(this.reactionSmarts);
				}

				MetaboliteFileReader metaboliteFileReader = new MetaboliteFileReader(is, metaboliteHandler);
       ...
       if (this.smirks != null) {
//         analyser.setReactionCentreTyper(new SmirksTyper(this.smirks));
       }
...
//     new ProgressMonitor(builder).start();
     try {
       ...
       System.out.println("Try increasing the amount of memory available to java.");
       System.out.println("java -Xmx[max memory]");
       System.out.printf("Current maximum memory: %.1fMB%n", new Object[] { Double.valueOf(maxMem) });
       System.out.println();
       throw e;
     }
     in.close();
