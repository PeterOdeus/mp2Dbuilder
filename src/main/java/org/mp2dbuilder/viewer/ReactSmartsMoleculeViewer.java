...
public class ReactSmartsMoleculeViewer extends MoleculeViewer {
	...
	public ReactSmartsMoleculeViewer(ReaccsMDLRXNReader reader, String filename)
			throws Exception {
	...
	protected void generateImage() throws Exception {
		...
		try {
			...
			IAtomContainer reactant = (IAtomContainer) reactionSet.getReaction(
					0).getReactants().getMolecule(0);
	...
	// ReactionSmartsQueryTool sqt = new
	// ReactionSmartsQueryTool(reactantQuery,productQuery);
	//		
	// //We know we only have one reaction in the reactionset
	// IReaction reaction = reactionSet.getReaction(0);
	//		
	// //It seems we need to do atom typing for SMARTS matching to work. TODO:
	// Confirm
	// IAtomContainer reactant = (IAtomContainer)
	// reaction.getReactants().getMolecule(0);
	...
	// reaction.getProducts().getMolecule(0);
