		...
		try{
			if (_viewer.PREVIOUS.equals(_cmd)) { //first button clicked
				if(_viewer.currentItemIndex == 0){
					JOptionPane.showMessageDialog(_viewer, "This is the first structure, starting from RIREG#" 
							+ _viewer.riregNoText.getText() + ".");
					return null;
				}
				tempCurrentRireg = _viewer.riregMap.get(_viewer.currentItemIndex - 1);
				_viewer.currentItemIndex -= 1;
			} else if (_viewer.NEXT.equals(_cmd)) { 
//				if(_viewer.currentItemIndex == (_viewer.reactionList.size()-1)){
//					JOptionPane.showMessageDialog(_viewer, "This is the last structure.");
//					return null;
//				}
				...
				if(_viewer.currentItemIndex >= _viewer.riregMap.size()){
					...
					tempCurrentRireg = _viewer.riregMap.get(_viewer.currentItemIndex);
				}else{
					tempCurrentRireg = _viewer.riregMap.get(_viewer.currentItemIndex);
					_viewer.tryToReset();
	...
	private void establishNextFilteredItem() throws ReaccsFileEndedException, CDKException, CancelledException{
		int i = 0;
